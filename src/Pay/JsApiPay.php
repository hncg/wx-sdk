...
use WxSdk\Pay\Lib\WxPayConfigInterface;
use WxSdk\Pay\Lib\WxPayException;
use WxSdk\Pay\Lib\WxPayJsApiPay;
...
class JsApiPay
{
	...
	/**
	 * 小程序统一下单,在调用GetJsApiParameters拿到参数之后,需要再再签名一次
	 * @param WxPayConfigInterface $config
	 * @param $unifiedResult
	 * @return string
	 * @throws WxPayException
	 */
	public function signAgain(WxPayConfigInterface $config, $unifiedResult)
	{
		//签名步骤一：按字典序排序参数
		ksort($unifiedResult);

		$buff = "";
		foreach ($unifiedResult as $k => $v)
		{
			if($k != "paySign" && $v != "" && !is_array($v)){
				$buff .= $k . "=" . $v . "&";
			}
		}
		$buff = trim($buff, "&");
